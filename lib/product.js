...
import { filterVisibleToBusiness, isVisibleToBusiness } from '@/lib/access-control';
...
export async function getProductData(handle, selectedOptions) {
  ...
  // customer_only products are reserved for the retail storefront — treat them as
  // not found here rather than rendering a product page a business buyer can't check out.
  const productVisible = data?.product && isVisibleToBusiness(data.product.tags);

  return {
    ...data,
    product: productVisible ? data.product : null,
    products: data?.products ? { ...data.products, nodes: filterVisibleToBusiness(data.products.nodes) } : data?.products,
  };
}
